...
1. **The tenant-partitioning technical decision** (Open Question #15) —
   Technology Baseline provides evidence (R4, PostgreSQL RLS), not the
   decision.
...
3. **Actual exit-strategy procedures** for the 7 Tier-1 Engines (R-08) —
   fallback candidates are named, procedures are not.
...
5. **Verification of Arabic/RTL localization support** for each
   ...
   (`docs/api-platform/31`'s conditional recommendation).
6. **A decision on the Eramba Community reconsideration** (R-01) — the
   ...
   phase has declined to perform.
7. **API Gateway, Secrets/Vault, Developer Portal product selections**
   (Open Questions #28-30) — each via a scoped Build-vs-Buy micro-
...
10. **A dedicated one-page Domain Vision Statement** for the Core
    ...
    `05-DDD-CERTIFICATION.md` finding).
