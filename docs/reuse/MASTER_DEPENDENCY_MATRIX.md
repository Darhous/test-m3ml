...
| ERPNext | purchase-requisition-order, supplier-rfq, receiving-goods (composed with OpenBoxes) (Procurement), supplier-registry-evaluation, contract-management (Supplier Management), invoicing-engine, pricing-price-lists, collections-dunning (Billing), cashbox-treasury-management, refund-processing (composed) (Payments and Treasury) | Procurement, Supplier Management, Billing, Payments and Treasury | `purchase-requisition-order` (Module 19) — now spans 4 Modules, the broadest single-Engine footprint in the program after Keycloak |
