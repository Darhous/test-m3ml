...
| P12 | Emergency/Break-Glass Actor | A P1/P2/P5/P6 identity operating under an exceptional, time-limited, justified, fully-audited elevated path | Same authentication boundary as the underlying identity, plus Break-Glass activation (§20) | Cross-tenant/cross-scope, exceptionally | Same as the underlying identity's own lifecycle owner | Same as underlying identity | Same channels, via the Break-Glass path | Always — this *is* the elevated-risk condition | Elevated, separately logged (§19, §20) | Accepted Rule (Constitution §21); technical enforcement mechanism Open (Wave 7 SEC-03, §32 item 14) |
...
| P14 | Governance/Oversight Persona (Auditor, Compliance Staff, Legal Reviewer, Regulator) | Read-only by design (Auditor), or narrowly-scoped review access | Keycloak-issued OIDC session | Cross-tenant where oversight requires it (e.g., platform-wide Auditor), Tenant-scoped otherwise | Platform Operations | Keycloak | Web Platform (audit/compliance surface) | Access to Sensitive Personal/Health data for audit purposes | Elevated (their own access is itself audited) | Accepted Constraint (category) |
...
| Governance/Oversight Persona (P14) | Keycloak | Platform Operations | Keycloak | Platform Operations | Identity and Access | Platform Operations (cross-tenant where applicable) | Platform Operations | Platform Operations | Audit and Compliance (their own access is itself audited) | None identified |
...
| F9 | Break-Glass authentication | P12 | Elevated, exceptional path | TB-14 | Same as underlying identity + activation justification | Time-limited, justified | Cross-tenant/cross-scope, exceptionally | Explicitly logged, not silently denied nor silently allowed | Technical enforcement mechanism Open (Wave 7 SEC-03) |
...
| Tenant Context | Token claim, cross-checked against `X-Tenant-Id` header; mismatch rejected outright (`14-MULTI-TENANCY.md`, restated Wave 7 §7) | Trusted — never client-supplied alone |
| Organization Context | Same pattern as Tenant, nested | Trusted |
| Branch Context | Same pattern, nested | Trusted |
...
| Administrative console | Elevated | Break-Glass, elevated administrative actions | P5/P6, elevated authentication | Cross-tenant where the action requires it | Fail closed, mandatory audit | Mandatory (Constitution §21) | Elevated authorization required for every action | Technical Break-Glass enforcement mechanism Open (§20) |
...
| Governance/oversight roles | Cross-tenant or Tenant-scoped per persona | Auditor (read-only by design), Compliance Staff, Legal Reviewer, Regulator (P14) | Auditor is read-only by design (Discovery persona catalog) | Not Tenant-customizable for the Auditor's read-only floor |
...
| AI recommendation acceptance | Human reviewer identity (P1/P4 subset) | HITL gate (Accepted, Constitution §28) | N/A | Standard (AI action logged) | Implicit | N/A | See Wave 7 §17 |
...
| Review | **Open — not established by any source** | No source describes a post-hoc review process for Break-Glass usage |
...
|---|---|---|
| Platform Administrator (P6) | Cross-tenant, highest privilege | **Recommended**: peer/dual-control for the most sensitive platform-wide actions — not yet a Constitution-level rule; a single Platform Administrator holding unchecked cross-tenant authority is a residual risk, not resolved here |
...
| Element | Status | Detail |
|---|---|---|
| Establishment | Accepted Rule | Token claim, established at login (§5) |
...
| Server-side resolution | Accepted Rule | Every Data-Scope PEP resolves Tenant server-side (§11) |
...
| Validation at each boundary | Accepted Rule | Cross-checked (token claim vs. header), rejected on mismatch (`14-MULTI-TENANCY.md`) |
...
| Stale context | **Open — not established by any source** | Whether a session's cached Organization/Branch context is re-validated periodically is not designed |
| Deep link | **Open — not established by any source** | Behavior when a URL references a resource outside the session's current active context is not designed |
| Background tab/session | **Open — not established by any source** | Not addressed by any source |
...
**Clarification (Reader Testing Pass 3, §48)**: Tenant context is always re-asserted and cross-checked per request (token claim vs. header, §5/§10/§29 — Accepted, no ambiguity). **Organization/Branch "active context" is session-scoped** (this section's own "Active context" row) — whether it is additionally re-validated per request the way Tenant is, or trusted from the session for its duration, is genuinely **not decided by any source** and was not previously tracked as an Open Decision. It is added to §43 (item 16) by this correction, since it is a materially different isolation posture depending on the answer (a stale session-cached Branch context could grant access to data outside a user's current, intended scope if a Branch reassignment happens mid-session).
...
| Human reviewer identity | Accepted Rule | HITL requires an authorized human reviewer identity (P1/P4 subset) — exact reviewer-eligibility Role not designed |
| Approval authority | **Open — not established by any source** | Whether the reviewer must hold a specific Role beyond "any authorized human" is not designed |
...
| Offline Collector (cross-cutting) | Inherits whichever central tier the sync target uses | N/A (device-local) | N/A | N/A | N/A | N/A (no server-side secret held on-device) | N/A | Not confirmed as an Audit Event at sync time (§34) | N/A | Mandatory pre-production blockers apply regardless of mode (§34) |
...
flowchart TB
    ...
    BG -->|time-limited| Elevated[Elevated, cross-scope access]
...
| 16 | Organization/Branch active-context freshness during a session (added, Reader Testing Pass 3, §48) | Open — not established by any source | Medium-High — a stale session-cached Branch context could grant access outside a user's current intended scope if reassignment happens mid-session | Blocks a complete answer to whether Organization/Branch context receives the same per-request re-validation rigor Tenant context does | Architecture Review Board | No | No | Wave 8 revision, Implementation | No |
| 17 | AI Human-in-the-Loop reviewer-eligibility Role (added, Reader Testing Pass 4, §48) | Open — "any authorized human" (P1/P4 subset) is not narrowed to a specific eligible Role | **Highest — direct clinical-safety impact**: as stated, the HITL requirement (Constitution §28, CLAUDE.md Rule 15) could structurally be satisfied by a reviewer without the clinical qualification to meaningfully review the specific AI output (e.g., a Patient identity is within the stated P1/P4 subset) | Blocks a defensible claim that Human-in-the-Loop review is clinically meaningful, not merely procedurally present | Architecture Review Board / Clinical Governance | Possibly | Possibly (clinical governance/licensure) | Wave 9 | Yes, for a defensible HITL clinical-safety claim |
...
Checked every "Accepted"/"Frozen"/"Required" label in §4–§37 against its cited source. Result: **zero new architectural decisions introduced.** Two items checked specifically for New-Decision risk:

1. **The hybrid Policy-Based Access model (§9)** restates ADR-0008's own "Role + Permission + Policy + Data Scope + Organization + Branch + Resource Ownership + Consent" framing — it is a restatement applied to the IAM-governance layer, not a new architectural decision, and this Wave explicitly declines to name it a new pattern (e.g., "ReBAC") beyond what the sources support.
2. **The Domain Invariant Separation Matrix (§13)** applies Wave 4/5's own Aggregate-invariant concept and Wave 7 §14's own "Aggregate protects invariants after authorization context" statement — it is a restatement/tabulation, not a new rule.
...
See §45 — zero new architectural decisions found.
