...
| Non-repudiation (where applicable) | A Sensitive Operation's actor cannot credibly deny performing it, given the immutable Audit trail | Accepted Security Rule | Constitution §23; `09-AUTHORIZATION.md` |
...
|---|---|---|---|
| Human users (clinical/patient-facing) | Patients, Doctors, Laboratory Staff, and similar direct-care/direct-use personas | Keycloak-issued OIDC session, Authorization Code + PKCE | `08-AUTHENTICATION.md` |
...
| Service identities | Every Module and Independent Component's own dedicated identity | Client Credentials grant | `08-AUTHENTICATION.md` |
| Device identities | Lab analyzers, portable collection devices | Mediated entirely through Device Integration Gateway's own Anti-Corruption Layer; never directly Keycloak-authenticated | `08-AUTHENTICATION.md`; ADR-0006 |
| Partner systems | External business-relationship machine-to-machine callers (FHIR/insurance/payer integrations) | Client Credentials, contract-scoped | `08-AUTHENTICATION.md` |
...
| API Edge zone | Kong Gateway | Platform Operations | Trusted to enforce coarse AuthN/RBAC and rate limiting; not trusted with fine-grained Data Scope decisions | From Untrusted/Client-device/Partner/Device zones | To Central Application zone | Passes through all classes; holds none authoritatively | OIDC/JWT validation, coarse RBAC, rate limiting, schema validation | Uniform across modes (Kong self-hostable, ADR-0009) |
...
| TB-05 | Central Application | Primary Data (PostgreSQL) | All data-touching scenarios | All Sensitive Personal/Health, Confidential classes | Module's own DB credential (per-Module, Schema per Module) | RLS + tenant-ID discriminator (ADR-0013) | At rest — ADR-0014 encryption posture; no algorithm chosen (§31) | Parameterized queries assumed (no SQL-injection-specific control named by any source — flagged, §11) | Not itself an Audit Event (Sensitive Operations audited at the Module/Aggregate layer, not the DB call layer) | Transaction atomicity preserved (Wave 6 §32) | RLS bypass, missing tenant filter (§12) | Wave 8 (identity), implementation (query layer) |
| TB-06 | Central Application | Platform Infrastructure (Keycloak/OPA/OpenBao/RabbitMQ/Unleash) | All | Security Secret, policy decisions | Service identity, Client Credentials | Least Privilege service accounts | In transit (internal) | N/A (structured internal protocol per product) | Secret access is itself logged as an Audit Event (`12-SECRETS-AND-KEYS.md`) | Fail closed (deny by default on Keycloak/OPA unavailability) | Compromise of any one is platform-wide (§10) | Wave 8 |
...
| TB-11 | Central Application | External Partner/Payer zone | R9, R11 | AS-02 (result exchange), AS-10 (claims) | Client Credentials, contract-scoped | Partner-contract-scoped Data Scope | In transit | Partner-supplied FHIR payloads validated at the Module boundary | Cross-tenant/cross-partner access is audited (mirrors Break-Glass treatment, `14-MULTI-TENANCY.md`) | Claim/exchange remains pending on failure (Wave 6 §32) | Partner credential compromise, webhook authenticity (§22) | Wave 8 |
...
| TB-14 | Platform Operations/Admin | Every other zone | Administrative/support scenarios | All classes, by necessity | Elevated authentication | Elevated authorization, Break-Glass discipline where cross-tenant | N/A (internal) | N/A | Mandatory (Constitution §21 Break-Glass requirement) | Break-Glass is time-limited, justified, audited | Standing-override misuse if discipline lapses (§11 THR-003, §32 item 14) | Wave 8 |
...
flowchart LR
    ...
    Kong4 --> AdminAPI
    AdminAPI -.->|exceptional, cross-tenant| BreakGlass
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| THR-001 | Denial of Service | API Edge zone (platform-wide) | TB-01 | Untrusted Internet → API Edge | All | No platform-wide rate-limiting policy fully implemented | Unthrottled requests degrade service for all Tenants sharing infrastructure | Service degradation, cross-tenant availability impact | Rate limiting/circuit-breaking named (`13-RATE-LIMITING.md`, `10-API-GATEWAY.md`) | Numeric thresholds Deferred; traffic-anomaly monitoring not yet designed | **Open** — High | Not yet defined | SEC-01 |
...
| THR-012 | Tampering | Immutable Audit zone | TB-07 | Central Application → immudb | Any Sensitive Operation | Elevated-access actor attempts to alter/delete an Audit Event | Audit tampering | Loss of the platform's core accountability guarantee — undermines every other mitigation in this register | Append-only enforcement at the audit store level (Constitution §23, Accepted); immudb's cryptographic tamper-evidence ("holds even against a malicious DBA" — Reuse research) | SAD-level verification that append-only enforcement is correctly configured, not yet performed | **Open, low residual if correctly configured** — Highest impact if it fails | Any successful mutation attempt should be structurally impossible | SEC-13 |
| THR-013 | Spoofing | API Edge zone | TB-01, TB-02 | Untrusted Internet → API Edge | All | — | An attacker impersonates a user or service identity | Unauthorized access at whatever Data Scope the impersonated identity holds | OIDC/JWT, mTLS-backed Client Credentials (`11-API-SECURITY.md`) | Token TTL and mTLS rollout specifics Deferred (§21, §31) | Open — Medium | Security tests attempting role/identity forgery (Constitution §20) | New (STRIDE sweep) |
...
No numeric risk score is invented anywhere in this table — severity follows the qualitative High/Medium/Low convention already established by the Discovery Risk Register and Wave 6.
...
| Human/service/device identity distinction | Accepted Architectural Constraint | Human identities: Keycloak-managed OIDC sessions. Service identities: dedicated Client-Credentials-based identity per Module/Independent Component, never sharing credentials with a human identity. Machine (device) identities: never directly Keycloak-authenticated — mediated entirely through Device Integration Gateway's own Anti-Corruption Layer (`08-AUTHENTICATION.md`) |
...
| Fail-closed behavior | Accepted Security Rule | Identity/Policy (Keycloak/OPA) unavailability results in full denial, consistent with Deny by Default (Wave 6 §32) |
...
| Rate limiting status | Recommended Security Practice, numeric values Deferred | Category named (`13-RATE-LIMITING.md`); per-Tenant tracking so one Tenant's traffic cannot degrade another's (`14-MULTI-TENANCY.md`) |
...
| Partner/Public API security | Accepted Architectural Constraint, product-level detail Deferred | Client Credentials, contract-scoped (`08-AUTHENTICATION.md`); no Public/anonymous developer access exists in current scope (`19-WEBHOOKS.md` Subscriptions note) |
...
| Device identity | Accepted Architectural Constraint, mechanism Deferred | Devices are not directly Keycloak-authenticated; mediated entirely through the Gateway's own credential/protocol scheme (`08-AUTHENTICATION.md`) |
...
|---|---|---|
| Device loss/theft | **Open — THR-021, High** | No confirmed control; local storage mechanism itself is Deferred (Wave 6 §20) |
| Unauthorized local access | **Open** | Same root cause as device loss/theft — access-control mechanism for the local store not designed by any source |
| Local data confidentiality | **Open** | Encryption-at-rest for the local store not designed by any source |
| Tampering | **Open — THR-020, High** | Local-record integrity/authenticity mechanism not designed by any source |
| Replay | **Open** | Duplicate-synchronization handling not designed by any source (see also Wave 6 §37) |
| Duplicate synchronization | **Open** | Same as above; server-side idempotency at sync time not explicitly designed for this specific flow (general Idempotency Policy, Constitution §48, would apply in principle) |
| Conflict | **Open** | Conflict-resolution algorithm not specified by any source (Wave 6 §20) |
| Device/user identity | Accepted Architectural Constraint, mechanism Deferred | The Collector operates as an authenticated human identity (§6); the *device's own* identity/binding to that user is not separately designed |
| Sync authenticity | **Open — TB-03** | "Authenticity check mechanism Deferred" (Wave 6 §20, restated here as a trust-boundary requirement) |
| Local retention/minimization | **Open** | No retention window or minimization rule for locally-cached data is stated by any source |
| Remote revocation/wipe requirement status | **Open, not established by any source** | No MDM/remote-wipe product or capability is named anywhere — this Wave does not invent one |
| Evidence preservation | Partially covered | Server remains authoritative after reconciliation (Wave 6 §20); provenance requirement (Constitution §24) would extend here in principle, but is not explicitly re-stated for the offline case by any source |
...
|---|---|---|---|---|---|---|---|---|---|
| PostgreSQL (platform-owned Module schemas) | Owning Module only (Schema per Module, ADR-0003) | Owning Module (direct); other Modules via API/Event/Read Model only | Platform Operations | RLS + tenant-ID discriminator, Accepted (ADR-0013) | At rest per ADR-0014 posture; algorithm not chosen (§31) | Transaction atomicity (ADR-0013); no partial commits | Not itself an Audit Event trigger (Sensitive Operations audited at Module/Aggregate layer) | TB-15 (§8) | SQL-injection-specific control not named by any source |
...
| immudb (Immutable Audit) | Every Module producing a Sensitive Operation/AI action/Break-Glass event | Authorized audit/compliance review roles | Audit and Compliance | Tenant-attributed within records; store itself is platform-shared | At rest (immudb's own); cryptographic tamper-evidence via Merkle-tree structure, described in Reuse research as holding "even against a malicious DBA" | Append-only enforcement (Constitution §23) — the store's core security property | This *is* the Audit mechanism | TB-15 | Fail-closed-vs-queue policy on unavailability Open (§11 THR-022) |
| OpenBao (Secrets) | Platform Operations, per-service scoped write | Only the Module/service that needs a given secret | Platform Operations | Cross-tenant infrastructure; per-Tenant secrets scoped within by convention, not independently verified by any source | At rest, independent of application-database encryption (`12-SECRETS-AND-KEYS.md`) | Every secret read is itself an Audit Event (`12-SECRETS-AND-KEYS.md`) | Yes — read access itself audited | TB-15 | Rotation cadence not fixed (No-Guessing Rule); recovery mechanism Deferred (Wave 6 §42) |
...
| Audit | Accepted Security Rule | Every read of a secret is itself an Audit Event (`12-SECRETS-AND-KEYS.md`) |
...
| immudb | Frozen Technology Input | The Audit and Compliance store; cryptographic tamper-evidence (Reuse research) |
...
| SEC-CTL-016 | Immutable Audit Trail (immudb, append-only) | Guarantee Sensitive Operations cannot be repudiated or the record tampered with | Immutable Audit zone | Accepted Security Rule | THR-012, THR-015 | Constitution §23, Reuse research (cryptographic tamper-evidence) | Audit completeness/immutability tests | Audit and Compliance | Fail-closed-vs-queue on unavailability (THR-022) |
...
| SEC-CTL-019 | Secret Read Auditing | Detect and attribute every secret access | Platform Infrastructure zone | Accepted Security Rule | THR-018 (partial) | `12-SECRETS-AND-KEYS.md` | Audit log review | Platform Operations | — |
...
| THR-002 | None | Open | High — no incident-response process defined anywhere | §30 below; Wave 12 |
...
| THR-010 | None dedicated (BR-INV01 reason-code rule, outside this catalog's Central-Application-security scope) | Partial (business-rule level) | Medium — periodic audit process not designed | Wave 12 |
| THR-011 | None dedicated (segregation-of-duties, not yet a Constitution-level rule) | Open | Medium | Wave 12 |
...
| THR-014 | None dedicated (TLS stated as semantic requirement only) | Partial | Medium — TLS version/cipher not chosen | §31 (deliberate non-decision) |
...
| THR-020 | None (local-storage integrity mechanism Deferred) | Open | High | Wave 9 |
| THR-021 | None (local-storage encryption/remote-wipe Deferred) | Open | **High** | Wave 9 |
...
|---|---|---|---|---|
| `stride-analysis-patterns` | `SKILL.md` (this session) | 6 STRIDE categories, Threat Analysis Matrix questions | §11, §12 | Every entity/process/store/flow/boundary in §7–§9 systematically checked against all 6 categories, not just an ad hoc list; THR-001–THR-024 each carry an explicit STRIDE category |
...
All 6 STRIDE categories (Spoofing, Tampering, Repudiation, Information Disclosure, Denial of Service, Elevation of Privilege) appear at least twice each across THR-001–THR-024 (checked by direct count against §11's own category column). **Corrected (Reader Testing Pass 2, §35)**: this Wave's initial self-review overstated coverage by claiming every §8 boundary has an associated Threat ID. Re-checked directly: TB-06 (Central Application → Platform Infrastructure), TB-09 (Central Application → Device Integration Gateway via broker), and TB-13 (Central Application → Messaging Carrier) have no THR row citing them explicitly in §11's Boundary column — their risk is covered only indirectly (e.g., TB-06's platform-infrastructure-compromise risk is implicit in THR-003/THR-018's Central-Application-side entries, not stated at TB-06 itself). This is a genuine, disclosed gap, not silently corrected by inventing new threat rows: every other named entity/process/store/flow does have at least one Threat ID, and the 3 boundaries above are lower-risk, internally-mediated paths (not directly attacker-reachable) — recorded as a residual documentation gap, tracked in §32.

### Threat-Mitigation Coverage

Every one of the 24 Threat IDs has a Coverage determination (Complete/Partial/Open) in §28, with a residual-risk statement — none is left blank. 12 of 24 threats trace directly to the pre-existing Discovery SEC-01–SEC-13 register, preserving that register's own severity ratings rather than re-deriving them.
...
Checked every "Accepted"/"Frozen"/"Required" label in §3–§27 against its cited source. Result: **zero new architectural decisions introduced.** One item checked specifically for New-Decision risk and confirmed clean: SEC-CTL-006 (Engine Tenant-Isolation Due-Diligence Gate) applies Constitution §19/§36's existing isolation-testing *pattern* to Engines — it does not invent a new testing methodology or select a new tool; the gate itself is a due-diligence checkpoint, not a component. The Elevated Audit tier's precedence correction (§14) promotes a status using an already-Accepted Decision (D-52), not a new decision authored by this Wave.
...
| F | STRIDE applied to entities/processes/stores/flows | **PASS (with disclosed gap)** | §11 — all 6 STRIDE categories applied across 24 Threat IDs; §34 STRIDE Coverage subsection honestly discloses that TB-06/TB-09/TB-13 lack a directly-citing Threat ID (lower-risk, internally-mediated boundaries), tracked as a residual documentation gap rather than concealed |
| G | Every threat has a mitigation or explicit residual/open status | **PASS** | §11, §28 — all 24 Threat IDs carry a Residual status; none is blank |
