...
**Three foundational clarifications this Wave enforces throughout, restated from Wave 3 §7 and Wave 2 §8 and never contradicted below:**
...
**Gateway responsibilities, quoted from `10-API-GATEWAY.md`'s own existing "Gateway Responsibilities Summary" table, not new design by this Wave**: TLS termination, AuthN (token validation), coarse AuthZ (RBAC), routing, rate limiting, protocol/version transformation, opt-in caching, correlation-ID assignment. **Explicitly not in scope for the Gateway**: fine-grained ABAC/Data-Scope decisions, business-logic dispatch, distributed tracing, Sensitive-Operation/Human-in-the-Loop gating — all of which remain the owning Module's own Aggregate layer.
...
| 2 | Practitioner and Clinic Management | Recognized | Practitioner and Clinic Management | 1:1 | Inferred | `practitioner_clinic_mgmt` | SAD-level proposed design |
| 3 | Scheduling and Encounters | Recognized | Scheduling and Encounters | 1:1 | Inferred | `scheduling_encounters` | SAD-level proposed design |
...
| 8 | Quality Management | Recognized | Quality Management | 1:1 | Inferred | `quality_management` | SAD-level proposed design |
...
| 10 | Asset and Maintenance | Recognized | Asset and Maintenance | 1:1 | Inferred | `asset_maintenance` | SAD-level proposed design |
| 11 | Inventory | Recognized | Inventory | 1:1 | Inferred | `inventory` | SAD-level proposed design |
| 12 | Procurement | Recognized | Procurement | 1:1 | Inferred | `procurement` | SAD-level proposed design |
| 13 | Supplier Management | Recognized | Supplier Management | 1:1 | Inferred | `supplier_management` | SAD-level proposed design |
| 14 | Billing | Recognized | Billing | 1:1 | Inferred | `billing` | SAD-level proposed design |
| 15 | Payments and Treasury | Recognized | Payments and Treasury | 1:1 (Partnership with #14, W9) | Inferred | `payments_treasury` | SAD-level proposed design |
| 16 | Insurance and Corporate Contracts | Recognized | Insurance and Corporate Contracts | 1:1 | Inferred (BC); Evidenced (openIMIS adoption, §18) | `insurance_corporate_contracts` | SAD-level proposed design |
| 17 | Accounting | Recognized | Accounting | 1:1 | Inferred | `accounting` | SAD-level proposed design |
| 18 | Workforce Management | Recognized | Workforce Management | 1:1 | Inferred | `workforce_management` | SAD-level proposed design |
| 19 | Payroll | Recognized | Payroll | 1:1 | Inferred | `payroll` | SAD-level proposed design |
| 20 | CRM and Support | Recognized | CRM and Support | 1:1 | Inferred | `crm_support` | SAD-level proposed design |
...
| 22 | Document Management | Recognized | Document Management | 1:1 | Inferred — **weakest-evidenced of the 28** (Discovery's own Wave 3 label) | `document_management` | SAD-level proposed design |
| 23 | Analytics | Recognized | Analytics | 1:1 (Independent-Capable) | Inferred | *(none — reads only; owns no primary data, §17)* | SAD-level proposed design |
| 24 | AI Operations | Recognized | AI Operations Gateway | 1:1 (Independent-Capable) | Inferred (BC); Accepted (governance mechanism, ADR-0007) | `ai_operations` (audit trail only) | SAD-level proposed design |
| 25 | SaaS Commercial Operations | Recognized | SaaS Commercial Operations | 1:1 | Inferred | `saas_commercial_ops` | SAD-level proposed design |
...
| 28 | Audit and Compliance | Recognized | Audit and Compliance | 1:1 (Platform Kernel) | Inferred (BC split); Evidenced (Constitution §21-23 primitives) | `audit_compliance` | SAD-level proposed design |
...
**Genuine evidence gap, logged rather than resolved**: `docs/discovery/artifacts/W3-enterprise-capability-map.md` names two further candidate contexts — "Partner and Marketplace Operations" and "Platform Administration" — that never appear in the final W9 28-context list, in ADR-0012, or in `03-API-DOMAIN-INVENTORY.md`'s 28-Module classification. This Wave does not silently drop them or silently add them as a 29th/30th context; it is recorded here and in §24 as an unresolved evidence-trail gap for a future Wave or the Project Owner to address, not invented away.
...
| Analytics *(Independent Component, §13)* | Cross-domain read models | Owns no primary data itself (explicit in source) | Reads from every other context, Open Host Service + Published Language, read-only; never writes to a source context | — | Do not give this context write authority over any source data |
...
| **Analytics Platform** | Cross-domain read models, variable/read-heavy load isolation | Constitution §11 (general grant only) | Read-only Open Host Service to every other context | Owns no primary data (explicit in source) | Apache Superset (E10) | **Permitted** | Finalizes actual v1 deployment form |
...
│                                    a Controller/Inbound Adapter calls
├── Domain model            — Aggregates, Value Objects, Domain Events (own Ubiquitous Language)
...
**Schema ownership**: each of the 24 non-Platform-Kernel, non-Independent-Component-exclusive Domain Modules (and the 3 Platform Kernel contexts) owns a distinct logical schema, named in §9's "Owning schema" column (ADR-0003). **Writer ownership**: only the owning Module ever writes to its own schema; every other Module reaches that data only through the owning Module's API, its published Events, or an approved Read Model (ADR-0003, ADR-0004).

**Approved read models**: Analytics is the platform's own approved cross-context Read Model provider — "reads from every other context... never writes to a source context" (§9, §12). No other context is granted blanket cross-schema read access; a Module-specific Read Model (if any) requires its own explicit design, not invented here.

**Tenant-scoping principle** (restated from Wave 3 §11/§12, at Building-Block depth, not redesigned): every tenant-scoped query in the shared tier is additionally scoped by Row-Level Security + tenant-ID (D-42, R4) at the database layer; the API layer's tenant check (JWT claim + `X-Tenant-Id` header cross-check, `14-MULTI-TENANCY.md`) is a second, independent, defense-in-depth mechanism, not a substitute for the database-layer one.
...
Every Engine with an evidenced, module-level "Single Adoption Point" in `docs/reuse/MASTER_*.md` (read fresh for this Wave), mapped to its owning block:
...
- Resolution of the two unreconciled Discovery gaps: the "Partner and Marketplace Operations"/"Platform Administration" candidate contexts (§9), and the Specimen Operations/Laboratory Execution split's own flagged uncertainty (carried unresolved from Wave 3 §7, unchanged here).

## 24. Open Dependencies and Deferred Modeling

| Item | Status | Source | Impact on this Wave | Blocking? | Owner | Future Wave |
|---|---|---|---|---|---|---|
| "Partner and Marketplace Operations"/"Platform Administration" candidate contexts, absent from the final 28-context list | Open — genuine evidence-trail gap, not resolved by this Wave | `W3-enterprise-capability-map.md` vs. `W9-bounded-context-remapping.md` | None on this Wave's own mapping (§9 uses only the confirmed 28); a future context count could change if resolved | No | Architecture Review Board / Project Owner | Not tied to a specific Wave |
| Full tactical modeling (Aggregates/invariants) for the 19 Recognized-tier contexts | Open — explicitly deferred by ADR-0012 itself | ADR-0012 | This Wave's §12 stays black-box by design | No | Development Teams, per-context as reached | Wave 4 continues to own this at increasing depth in future iterations; not fully closed by this pass |
...
**Not re-introduced as Open**, consistent with Wave 1/Wave 2/Wave 3's own established corrections: Notification Channel Set, Legacy Migration, FHIR version (R4 pinned, R-06 Closed), the Independent Components' count (8, Constitution §11) — all remain correctly Resolved/Closed, not reopened by this Wave.
...
| Engine Adoption-Points (21 Engines) | Per §18's own rows | Per §18 | §5 | ADR-0001, ADR-0011, ADR-0012 | D-13-D-18 | R-01-R-08 | (21 of 24) | — | `MASTER_*` (all 18) | Wave 12 |
...
Every one of the 21 Engines with a module-level Adoption Point in §18 shows "Native API exposure: Forbidden," matching `01-API-VISION.md`/`02-API-FIRST-ARCHITECTURE.md`'s platform-wide rule with no exception carved out. The 3 Engines absent from `docs/reuse/`'s own research (Kong Gateway, OpenBao, PostgreSQL) are explicitly flagged as a research-coverage gap in their own §18 rows and in §24, not silently assigned an invented Adoption Point.
