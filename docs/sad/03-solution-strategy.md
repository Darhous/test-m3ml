...
2. **Selective independent components** — 8 named independent-capable components (Independent Components/Shared Services, Constitution §11) are permitted to be operationally independent from v1 for documented reasons (protocol isolation, external-facing surface, variable load, governed external calls); this permission is Accepted for all 8, but is not itself a decision that all 8 are deployed as separate units in v1 — the actual v1 deployment form of each is determined per component from its own governing ADR/Decision and finalized in Wave 6 (§26, Post-Review Semantic Erratum). Any other Module remains inside the single Modular Monolith deployable unless and until **Selective Service Extraction** — pulling it out into its own independently-deployed service — is justified by a measured, documented operational or organizational trigger and recorded in a new ADR (§18); no such extraction is planned or Accepted today. *[Constitution §5, §11; ADR-0001]*
...
### Accepted Strategy
**Modular Monolith First**, with 8 named independent-capable components (Independent Components/Shared Services, Constitution §11) permitted to be operationally independent from v1 for documented reasons, and Selective Service Extraction available for any other Module once a measured trigger exists. Logical and contractual independence for all 8 is Accepted; whether a given component is actually deployed as a separate unit in v1 is determined per component from its own governing ADR/Decision, not assumed uniformly here (§26, Post-Review Semantic Erratum).
...
One deployable unit (the Modular Monolith) hosts the majority of the 28 Bounded Contexts (Wave 2 §8), each with an enforced schema boundary (ADR-0003) and contract-only cross-Module access (ADR-0004). The 8 named independent-capable components are permitted to be operationally independent — contractually and logically separate from the Monolith, integrating with it and each other only through published APIs/Events, never a direct code dependency (Constitution §9, "Independent Components May Depend on Core Contracts Only") — but this permission does not by itself fix how many are actually deployed as separate units in v1. Their governing decisions differ per component, not uniformly: ADR-0006 states, in its own Negative Consequences, that the Device Integration Gateway adds "an operationally independent component to run/monitor from v1" — a stronger, component-specific commitment from that component's own governing ADR. No other of the 8 components' governing ADR (0007) or the Constitution §11 grant itself asserts an equivalent v1-deployment commitment; for those, Constitution §11 grants permission without mandating v1 deployment as a separate unit, and the actual v1 deployment form of each is determined per component and finalized in Wave 6. Logical architecture (Bounded Context boundaries, contracts, independence eligibility) is kept distinct from physical deployment topology (Wave 6's job) — "Modular Monolith" describes a dependency and contract discipline, not a literal single-process constraint, and "independent-capable" describes permitted architecture, not a fixed v1 deployable count (§26, Post-Review Semantic Erratum).
...
**Terminology, stated once here and used consistently throughout this Wave:** a **Bounded Context** is a Domain-Driven-Design modeling/language boundary — one of the 28 named contexts below — and is not itself a deployment concept (Constitution §6). A **Module** is the Modular Monolith's internal implementation unit (enforced schema boundary, ADR-0003; contract-only cross-Module access, ADR-0004); whether a given Bounded Context maps 1:1 to a Module is explicitly undecided (§21). An **Independent Component** is one of the 8 Constitution §11-named components **permitted** to be operationally independent — i.e., eligible to run as a separate deployable — for a documented reason (§6); this permission is Accepted for all 8 alike, but actual v1 deployment as a separate unit is determined per component from its own governing ADR/Decision (§26) and is not assumed uniformly here. A **Service** in this Wave means any independently-deployed unit — either one of the 8 Independent Components, or a Module later pulled out via Selective Service Extraction (§6, §18) once a measured trigger fires; it is never used here as a synonym for "Bounded Context" or "Module."
...
### Consequences and Trade-offs
Positive: new vendors/protocols are added at the Gateway without touching business Module logic; core data is protected from malformed device payloads by construction. Negative: per ADR-0006's own Negative Consequences section, this adds an operationally independent component to run/monitor from v1 — a commitment specific to this component's own governing ADR, not a general status this Wave asserts for all 8 Independent Components (§26, Post-Review Semantic Erratum); an Anti-Corruption Layer must be designed and maintained per vendor/protocol.
...
### Accepted Strategy
A **governed AI Gateway** (one of the 8 named independent-capable components, Constitution §11 — logically and contractually independent by Accepted permission; unlike the Device Integration Gateway's ADR-0006 commitment, ADR-0007 asserts no v1-deployment-specific claim for this component, §26) supporting summarization, patient-friendly explanations, pattern detection, anomaly detection, search, operational assistance, and clinical decision *support* — never final medical approval — with mandatory Human-in-the-Loop before any sensitive clinical action is finalized, and every AI action logged and auditable.
...
| Independent components vs. operational complexity | Protocol isolation, governed external calls, variable-load isolation | 8 named components permitted (not uniformly mandated) to be operationally independent; only the Device Integration Gateway's own governing ADR (0006) commits it to run/monitor from v1 — the other 7 components' actual v1 deployment form is per-component, not fixed here | Coordination overhead across however many of the 8 are actually deployed separately, up to a pure-monolith baseline of zero | Constitution §11 (contract-only consumption); ADR-0006 (Device Integration Gateway specifically) | Actual per-component v1 deployment decisions and operational tooling/monitoring maturity are Wave 6/9 work | Constitution §11; ADR-0006 |
...
- Containers, Components, or any deployable-unit count — including, for the 8 named independent-capable components, whether each is actually deployed as a separate unit in v1. Permission to be operationally independent is Accepted for all 8 (Constitution §11); the Device Integration Gateway's own governing ADR (0006) additionally commits it specifically to v1 operational independence; no equivalent per-component commitment is asserted here for the remaining 7, and the actual v1 deployable-unit count is finalized in Wave 6 (§26, Post-Review Semantic Erratum).
...
Following this self-review, an **Independent Architecture Review** examined this Wave and returned a verdict of **PASS WITH MANDATORY NARROW PRE-ACCEPTANCE ERRATUM**: a status-promotion defect in how this Wave described the 8 Independent Components' operational-independence status. The defect, its governing sources, and every corrected passage are recorded in full in §26 (Post-Review Semantic Erratum) below. The correction is narrow (wording only, within this Wave's own text), does not reopen or re-run Reader Testing, does not change any Validation Gate's PASS result above (Gate G, "Independent Components preserve Required/Permitted/Deferred status per item," is the gate this erratum brings into full compliance — it was not separately gated in the original 16-gate run because Wave 3 used a 16-gate, not 21-gate, template), and does not alter this Wave's Final Verdict of PASS. Wave 3's formal Project-Owner acceptance is recorded in a separate, subsequent commit only after this erratum's targeted verification (§26) passed.

## 26. Post-Review Semantic Erratum

**The prior error.** Constitution §11 (and its own glossary entry, "Independent Component") states the 8 named components are *"permitted to be operationally independent from v1 for a documented reason,"* not that all 8 are, or must be, separate deployables in v1. Several passages in this Wave's original text (§3 point 2, §6 Accepted Strategy, §6 How It Works at Strategy Level, §7's terminology definition, §19's Trade-offs table, §21's Explicit Non-Decisions bullet) flattened this permission into an asserted, uniform fact — using language such as "operationally separate from v1," "run as separate deployables from v1," "is operationally independent — a separate deployable — from v1," and "8 additional operationally independent components to run/monitor from v1." This is a **Status Promotion**: an architectural *permission* (Logical independence, Contractual independence, Permission to be operationally independent) was raised to a collective *Deployment Decision* (Mandatory independent deployment, Actual v1 deployment topology) without a source for that collective promotion.

**The governing source.** Constitution §11 and its Independent Component glossary entry ("permitted to be operationally independent from v1... without being a general microservices default") — read fresh for this correction, together with ADR-0001 (Modular Monolith First), ADR-0006 (Independent Device Gateway), ADR-0007 (Governed AI Gateway), the Unified Decision Register (`10-DECISION-REGISTER.md`, D-07 — no per-component v1-deployment decision exists there beyond citing ADR-0007), and Wave 2 §5/§8 (which independently use similar "operationally independent from the start" language — itself Wave 2's own Accepted text, out of this erratum's scope-lock and not altered here). One material asymmetry was found in the ADRs themselves: **ADR-0006's own Negative Consequences section states, in its own words, "Adds an operationally independent component to run/monitor from v1"** — a component-specific commitment from that component's own governing, Accepted ADR. **ADR-0007 (AI Gateway) contains no equivalent v1-deployment claim** — it names the AI Gateway as "one of the Independent Components" without asserting a deployment commitment. No Decision Register entry (D-01–D-59) asserts a collective v1-deployment decision for all 8 components as a group.

**Positions corrected.**
- §3, Executive Solution Strategy, point 2.
- §6, Overall Architecture Style — Accepted Strategy and How It Works at Strategy Level.
- §7, Domain-Driven Strategy — Terminology note's definition of Independent Component.
- §14, Device Integration Strategy — Consequences and Trade-offs (retained ADR-0006's own component-specific commitment, with an explicit disclaimer that it is not a general 8-component rule).
- §19, Strategy Trade-offs — the "Independent components vs. operational complexity" row.
- §21, Explicit Non-Decisions — the deployable-unit-count bullet.
- §15, AI Integration and Governance Strategy — Accepted Strategy header (a lighter, stylistic-consistency fix: this passage never asserted a v1-deployment fact, but lacked the explicit permission/no-ADR-0006-equivalent disclaimer §14 carries for the Device Integration Gateway; added for full terminology consistency, found during this erratum's own independent verification pass, §26 below).

**Independent-capable vs. independently deployed.** "Independent-capable" (or "permitted to be operationally independent") describes an **Accepted architectural eligibility**: a component may be built and contracted as a separate deployable, integrated only through published APIs/Events, never a direct code dependency. "Independently deployed" describes a **physical deployment fact**: that a specific component actually runs as its own process/service in a given environment at a given time. All 8 named components hold the first status, Accepted by Constitution §11, as of this Wave. Only one — the Device Integration Gateway — holds the second status as a documented commitment of its own governing ADR (0006), effective from v1. The other 7 hold only the first status per this Wave's reviewed sources; whether any of them is independently deployed in v1 is not decided by this Wave and is not invented by this erratum — it is determined per component from that component's own governing ADR/Decision (if any exists) and finalized in Wave 6 (Deployment View).

**This correction does not change Modular Monolith First.** ADR-0001 remains the governing decision for §6; the Modular Monolith remains the default deployable unit for every Module not named as one of the 8 independent-capable components or later extracted via Selective Service Extraction. Nothing in this erratum revisits, weakens, or reinterprets ADR-0001.

**This correction does not cancel any component-specific independence an ADR already imposes.** ADR-0006's Device Integration Gateway commitment ("to run/monitor from v1") is preserved exactly, in the component's own §14 section — this erratum corrects only the *general, all-8* claims this Wave had additionally and separately made elsewhere, which had no equivalent source.
