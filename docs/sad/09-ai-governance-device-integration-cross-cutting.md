...
| Review authority | Project Owner, per this task's own governing instruction's Formal Acceptance gate (§B) |
...
**Formally Accepted.** See §39 Formal Acceptance Record for the review basis, corrective commits, and what this acceptance does and does not mean.
...
| Portable Collection Device (Home Visit) | Device Integration Gateway | Inbound (specimen-collection events) | AS-15 (Offline local data) | Attributed post-ingestion, tenant established at Collector login (Wave 8 §34) | Vendor API (real-time), File-Based (batch sync) — **the specific assignment of these protocols to this device category is Draft-sourced (Discovery `08-integration-inventory.md`), not independently re-confirmed by a higher-tier source; this is distinct from File-Based Integration's own general status as an Accepted-supported protocol type at the architecture level (ADR-0006, §9) — the protocol type is Accepted, its assignment to Portable Collection Devices specifically is not (clarified — closes an inconsistency found by independent Device Integration review)** | TB-03 (Offline Collector-device → API Edge) | Protocol category candidates only for *this device category's own assignment*, not fully Confirmed — see §31 |
...
| File-Based Integration | **Accepted at architecture level as a supported protocol type — sourced from ADR-0006 alone, NOT Open Question #5 (which confirms only HL7v2/ASTM/Vendor API; clarified — closes an inconsistency found by independent Device Integration review).** Its *assignment* to Portable Collection Device batch sync specifically is a separate, Draft-sourced claim (`08-integration-inventory.md`, see §5), not independently re-confirmed | Inbound, batch | File-schema validation at ingestion | N/A (file-transfer-level, not message-level) | File-sequence-dependent, not designed | Deferred Detail | Deferred Detail | Depends on the file-transfer channel chosen — not designed | Established at provisioning | Constitution §24 | Malformed file quarantined | Not designed | ADR-0006 |
...
| Sync-time revalidation | **SAD-Level Design (this Wave's own closure of the core blocker) — corrected from an earlier "Accepted Rule" label, found overstated by independent Governance review: its source, Wave 8 §29's Context Freshness Policy, is itself labeled "SAD-Level IAM Design" there, not Accepted, so a derivation from it cannot itself claim Accepted-Rule (Constitution/ADR-fixed) status per this Wave's own §3 Decision Status Model** | Every synced record's authorization context (Tenant/Organization/Branch membership, Role, current session validity) **must** be re-validated server-side at sync time before being accepted — this directly satisfies Wave 8 §29's Context Freshness Policy (production-blocking for Sensitive Operations and cross-scope actions, Wave 8 §49 B3 — every offline-synced record is treated as a cross-scope reconfirmation for this purpose, regardless of whether the specific operation is separately classified a Sensitive Operation under Wave 8 §18) applied to the offline path specifically. A stale or revoked Collector identity is rejected/quarantined at this point, not silently accepted because it was captured while "still valid". **Caveat found by independent review, not previously stated**: this control's own evidentiary strength depends on whether the sync-time accept/reject decision is itself an Audit Event — which remains genuinely Open (row below, and IF11 §12/§26). This Wave does not overclaim sync-time revalidation as a complete safety guarantee; it is the strongest available control given the offline constraint, not a fully audited one yet |
...
| Element | Status | Detail |
|---|---|---|
| Trust boundaries | Accepted Constraint (inherited) | TB-08, TB-09, TB-10, TB-11, TB-12, TB-13 (Wave 7 §8) govern every integration in this Wave — **TB-08 added, found missing from this enumeration by independent Adversarial review despite being used in Diagram 1 and IF14's own Engine Adapter interaction flow** — no new Trust Boundary is introduced — this Wave operates *within* Wave 7's own catalog |
...
| THR-024 (Partner endpoint spoofing/webhook tampering, TB-11) | Spoofing/Tampering | **Reconciled here — found missing from this Wave's own Threat Traceability table by independent Security review, despite Wave 7's Risk Register explicitly assigning its resolution to "Wave 8/9."** §18 (Webhook Architecture) restates its existing mitigation (HMAC signing, HTTPS-only, SSRF protection at registration) unchanged from Wave 7, and additionally proposes resolving its "Missing mitigation: Ownership itself unassigned" gap (this Wave's own Webhook Delivery ownership proposal, §18, pending Board confirmation). **THR-032 (below) is a refinement of THR-024 for the same TB-11 webhook boundary, not a duplicate or a replacement** — THR-032 adds the DNS-rebinding/TOCTOU angle THR-024 did not cover; THR-024 itself remains open pending Webhook Delivery ownership confirmation and mechanism ratification | Owning-component ratification (this Wave's proposal, not yet Board-confirmed), delivery-time SSRF revalidation (THR-032's own addition) | Not designed | Open — Medium-High, Partial (ownership proposed, not yet ratified; DNS-rebinding gap now disclosed via THR-032) | Wave 9 (ownership proposal); Implementation (mechanism ratification) |
...
| VF15 | An offline-revoked user's synced data is rejected/quarantined at sync time | **SAD-Level Design (this Wave's own closure; corrected from "Accepted Rule," found by independent Final Verification)** | §19 |
...
flowchart LR
    ...
    Kong6 --> Revalidate[Sync-time revalidation - SAD-Level Design]
...
- Whether webhook callback endpoints require delivery-time SSRF/DNS-rebinding revalidation, beyond the existing registration-time check (**added — closes a broken cross-reference found by independent Adversarial review**: §18/§28 THR-032 cite this section for the gap; this bullet is what they refer to).
...
| 10 | The 6 not-yet-designed Partner API candidates — resource-level design (**scope clarified — closes a boundary-blur found by independent Adversarial review**: this item covers the 6 candidates §17 lists as "0 designed"; the openIMIS-backed claims candidate is "partially real" per §17 and is referenced here only because its AGPL Legal Dependency is analogous, not because it is one of the 6) | Open (§17) | Medium-High, especially by analogy to the openIMIS-backed claims candidate's own Legal Dependency | Blocks confident Partner go-live for any of the 6 candidates | Owning Module per candidate | No | AGPL review (R-04) is a separate, already-tracked dependency for the claims candidate specifically, not for these 6 | Implementation | No |
...
|---|---|---|---|---|
| Sync-time revalidation as a SAD-Level Design for offline authorization (§19) | **SAD-Level Design (corrected — an earlier draft mislabeled this "Accepted Rule"; found by independent Governance review, since its own source, Wave 8 §29's Context Freshness Policy, is itself only "SAD-Level IAM Design," not Accepted)** | Directly derives from Wave 8 §29's Context Freshness Policy, applied to the offline path | A necessary application of an already-established SAD-Level requirement to a specific channel, not a new authorization mechanism and not a Constitution/ADR-fixed rule | Exact local-storage mechanism, conflict resolution |
...
- **Device protocol families**: **corrected (found by independent Device Integration review)** — this row previously bundled all four families under one "Open Question #5" attribution, which is inaccurate: **HL7v2, ASTM, and Vendor API** are preserved exactly as Open Question #5's own resolution states ("all three... confirmed as the supported protocol families at the architecture level"), not weakened or strengthened. **File-Based Integration is a separate claim**, sourced from ADR-0006 alone (which names it as one of the protocol/technology types the Gateway must support over time) — Open Question #5 never mentions File-Based. Per-device-model detail remains Deferred for all four, unchanged.
...
No status was silently strengthened or weakened anywhere in this Wave beyond the two explicit SAD-Level Designs documented in §33 (sync-time revalidation; HITL governance-path design), both shown with their own derivation and neither claiming Constitution/ADR-level Accepted status.
...
| Offline sync revalidation | Wave 8 §29/§34/§49 | H7, H11, H14 | Sample Collector | IF11 | TB-03 | P1/P2 subset | Re-validated at sync (SAD-Level Design) | THR-020, THR-021 | Sync-time revalidation (§19) | VF15, VF16 | **SAD-Level Design** | #4, #5, #6 |
...
§19 — every Wave 7/8 Mandatory pre-production blocker given an explicit disposition: Resolved by SAD-Level Design (local confidentiality/integrity/replay/duplicate semantic requirements, and — corrected, see below — sync-time revalidation), or an explicit, owned pre-production Open Decision correctly carrying Mandatory status where inherited from Wave 7 §18 (§32 items 4, 5, 15, 20) and correctly non-Mandatory where genuinely new to this Wave (item 6) — none left as a bare, unowned "Open" restatement (corrected, §38). Sync-time revalidation's status label was itself corrected from an overstated "Accepted Rule" to "SAD-Level Design" during Wave 9's own pre-acceptance review, since its source (Wave 8 §29's Context Freshness Policy) is itself only SAD-Level, not Accepted.
...
A seventh, independent fresh sub-agent — having authored none of the prior 6 passes or their fixes — was given the findings above and the current saved file, and instructed to navigate to each cited location and quote current text rather than trust the fix summaries. **Count correction, found by independent Adversarial review**: the 6 passes above individually enumerate 3 (Pass 2) + 4 (Pass 3) + 1 (Pass 4) + 5 (Pass 5) + 1 (Pass 6) = **14 distinct findings**, not the "11" this section previously claimed — the earlier count undercounted by conflating some findings that received a shared fix location with the number of fix-actions taken, rather than counting findings themselves. Result: **all 14 confirmed RESOLVED** with quoted evidence, including cross-references between §11/§16/§19/§20/§21/§26/§28/§32's own item numbers all resolving correctly. No finding was found Partially Resolved or Incorrectly Closed.
...
| 23 | Offline production blockers resolved or explicitly blocking | **PASS (corrected, §38)** | §19 — sync-time revalidation a SAD-Level Design (not Accepted Rule, corrected per independent Governance review); encryption (item 4), conflict-resolution (item 5), remote-wipe (item 15), and retention/purge (item 20) all explicitly Mandatory pre-production blockers with owners, matching Wave 7 §18's own classification; partial-sync/ordering (item 6) correctly non-blocking as a genuinely new, not-inherited gap |
...
**No other correction was required** — the remaining 22 of 23 cross-wave checks (**enumerated individually here, correcting an earlier compressed list that named only 20 items — found by independent Adversarial review**: (1) authentication status, (2) Tenant Context authority, (3) OPA's role, (4) RLS's role, (5) Domain invariant separation, (6) Break-Glass scope, (7) Regulator/Auditor access, (8) device identity, (9) Device Gateway sole ACL boundary, (10) service/partner identity, (11) event identity, (12) offline credential/storage status, (13) rate limiting, (14) webhooks, (15) FHIR R4, (16) Engine access/isolation split, (17) AI governance use-case split, (18) HITL reviewer framing, (19) audit/evidence, (20) backup responsibilities, (21) deployment-mode vocabulary, (22) Recommendation-status preservation) all passed cleanly with quoted matching evidence from both sides. (Check 13 of the original 23 — offline blockers — is the one that failed and was corrected above.) This correction alone did not change Document Status; Wave 10 remains not started.

**This §38 record, like §37's Final Verdict, predates formal acceptance.** After this correction, 8 further independent reviewers ran a dedicated pre-acceptance pass (Governance, Security, IAM/Tenant-Isolation, Device Integration, Offline, Clinical/Privacy, Adversarial, and a second Final Verification) and found 9 additional real defects, all fixed and re-verified (commit `47b278b`). See §39 for the acceptance decision that followed.

## 39. Formal Acceptance Record

This record documents Wave 9's formal acceptance under this task's governing instruction (§B, Formal Acceptance).

| Field | Value |
|---|---|
| Review basis | Original authoring (§1–§36, self-review PASS, 6 Reader Testing passes + Final Verification, 46 Gates, §37); Final Cross-Wave review finding and closing 1 contradiction (§38, commit `d6d5947`); a pre-acceptance status-promotion sweep across diagrams and text, including matching fixes to already-Accepted Waves 7/8 diagrams (commits `81b3b72`, `4a68a3b`); a dedicated 8-reviewer formal-acceptance pass (Governance, Security, IAM/Tenant-Isolation, Device Integration, Offline, Clinical/Privacy, Adversarial, Final Verification) verifying claims against original sources rather than trusting prior self-reports, finding and closing 9 further real defects (commit `47b278b`) |
| Corrective commits | `d6d5947` (offline blocker restoration), `81b3b72` (Wave 7/8 diagram consistency), `4a68a3b` (Wave 9 status-promotion sweep), `47b278b` (formal-acceptance review corrections) |
| Independent verification | Every corrective round above was independently re-verified by a reviewer who did not author the fix, by direct quotation against the saved file — not by trusting a fix summary. No finding was left Partially Resolved or Incorrectly Closed at the time of acceptance |
| Formal acceptance commit | `docs(sad): formally accept Wave 9` — see repository history for the exact SHA |
| Date | 2026-07-21 |
| Acceptance conditions satisfied | No Recommendation promoted to Accepted anywhere (verified by the Governance reviewer against 6+ source citations); no Open Decision hidden inside Accepted-labeled text (the self-minted "Accepted Rule" for sync-time revalidation was found and relabeled "SAD-Level Design"); no contradiction (File-Based protocol status, THR-024/THR-032 orphaning, TB-08 omission, and the offline-blocker downgrade were all found and fixed); no unsupported claim (the 14-vs-11 Reader Testing count was corrected); no product treated as a control (verified clean by the Security reviewer); diagrams consistent with text (2 Wave 9 diagram labels and 2 baseline-Wave diagram labels corrected); every Threat dispositioned (THR-024 added to the traceability table, THR-030's STRIDE category corrected); every blocker handled correctly (offline blocker matrix independently re-verified fully consistent with Wave 7 §18 by a dedicated Offline reviewer) |
| What this acceptance means | The Wave 9 architecture baseline — Integration Landscape, Device Integration Gateway boundary and identity/trust model, Device Protocol Architecture, Message Normalization and Correlation, the 15-flow Integration Flow Catalog, Event/Broker Architecture, Integration Contract Governance, FHIR Integration Boundary, External Partner Integration, Webhook Architecture (including this Wave's own ownership proposal), Offline Architecture, AI Integration Architecture (including the HITL governance path), Integration Security/Reliability/Observability, Deployment-Mode Differences, Responsibility Matrix, and Threat Model/Traceability — is accepted as the baseline for subsequent Waves |
| What this acceptance does **not** mean | It does not resolve any of the 20 Open Decisions (§32) — device credential scheme, Mirth Connect vs. Apache Camel ratification, Webhook Delivery ownership confirmation, offline encryption/conflict-resolution/remote-wipe mechanisms, AI HITL reviewer-eligibility values, wrong-identifier correlation detection, manual-entry attribution, and the rest remain genuinely Open with named owners. It does not certify legal/regulatory compliance. It does not resolve THR-024's own remaining mitigation gap (webhook ownership still pending Board confirmation) or any other Threat's residual risk. It does not authorize or pre-decide any Wave 10 implementation detail — those remain Wave 10's own territory, to be authored fresh against this accepted baseline. |
