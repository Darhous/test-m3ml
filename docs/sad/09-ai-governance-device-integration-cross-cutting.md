...
| Conflicts | Open — not established by any source | What happens when two Collectors (or a Collector and an online path) produce conflicting records for the same Specimen is not designed — genuine gap, recorded §31/§32 |
...
| Purge/retention | Open — not established by any source | Local-device data retention/purge-after-sync policy is not designed |
...
| 5 | Offline sync conflict-resolution mechanism | Open (§19) | Medium-High | Blocks a complete offline-safety assurance case; a silent-corruption failure mode (VF17) is possible without one | Architecture Review Board | No | No | Implementation | No — not part of Wave 7's original Mandatory blocker list; a genuinely new gap found during this Wave's own design work, correctly not equated with H11/H14's inherited Mandatory status |
