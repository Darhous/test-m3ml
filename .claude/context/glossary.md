...
Constitution معتمدة. "Core Domain" كان الاستثناء الوحيد (Open) حتى
2026-07-18، حيث تحدَّد عبر ADR-0011 **Accepted** — انظر السطر أدناه.
...
| Core Domain | الـBounded Context (أو أكثر) الذي تكمن فيه القيمة التنافسية/المميِّزة للمنصة، ويستحق أعمق استثمار في النمذجة | **Accepted (2026-07-18)** — "Patient-to-Result Orchestration"؛ كان Open سابقًا حتى تحدَّد عبر جلسة DDD مخصصة (انظر `open-questions.md` البند 14) | [0011](../../docs/adr/0011-core-domain-test-processing-and-result-verification.md) |
...
## Gap Closure Wave 8 — Ubiquitous Language Disambiguation (2026-07-16)

37 مصطلحًا عرضة للتعارض (Order, Request, Booking, Appointment, Visit,
Encounter, Sample, Specimen, Test, Analysis, Result, Report, Verification,
Approval, Publication, Invoice, Receipt, Collection, Payment, Expense,
Reagent, Consumable, Lot, Batch, Tenant, Organization, Branch, Facility,
Department, Employee, Practitioner, Doctor, Patient, Guardian, Subscriber,
Customer, Partner) تم توضيحها بالكامل — كل مصطلح مربوط بالـContext المالك
له، مع تمييز صريح للمرادفات الممنوعة (Forbidden Synonyms). التفاصيل
الكاملة في `docs/discovery/artifacts/W8-ubiquitous-language-expansion.md`.
هذا **يوسّع** قوائم Draft الموجودة أعلاه، ولا يستبدلها.

## Discovery Phase 05 — Candidate Aggregates and Value Objects

**Inferred — Industry Reference**، من `docs/discovery/artifacts/
05-candidate-aggregates.md`. مرشّحة، قبل ترسيم Bounded Contexts رسميًا.

| Term | Draft Definition | Status |
|---|---|---|
| TestOrder | Aggregate Root مرشّح يمثل طلب فحص وحالته | Draft — Inferred |
| Specimen | Aggregate Root مرشّح يمثل دورة حياة عيّنة واحدة | Draft — Inferred |
| TestResult | Aggregate Root مرشّح يمثل نتيجة فحص من الالتقاط حتى الاعتماد والإتاحة (Core Aggregate المرشّح) | Draft — Inferred |
| ProvenanceInfo | Value Object مرشّح لتتبع مصدر نتيجة قادمة من جهاز (يلبّي Constitution Section 24) | Draft — Inferred |
| Invoice / Claim | Aggregate Roots مرشّحان منفصلان لدورة الفوترة والمطالبات | Draft — Inferred |
| DeviceImportRecord | Aggregate Root مرشّح يحمل البيانات الخام المستوردة من جهاز قبل عبورها إلى TestResult عبر Anti-Corruption Layer | Draft — Inferred |

## Discovery Phase 06 — Candidate Bounded Contexts (formalized)

**Inferred — Industry Reference**، من `docs/discovery/artifacts/
06-bounded-contexts.md`. هذه الحدود **مرشّحة** — تنتظر Phase 11/12.

| Term | Draft Definition | Status |
|---|---|---|
| Order Management (Bounded Context) | حدود نموذج تدير طلب الفحوصات وكتالوجها | Draft — Inferred |
| Specimen Management (Bounded Context) | حدود نموذج تدير دورة حياة العيّنة الكاملة | Draft — Inferred |
| Test Processing and Result Verification (Bounded Context) | حدود نموذج تدير المعالجة والاعتماد والإتاحة — **Core Domain مرشّح** | Draft — Inferred |
| Notification and Communication (Bounded Context) | حدود نموذج تدير إرسال الإشعارات (يقابل Notification Service) | Draft — Inferred |
| Billing and Claims (Bounded Context) | حدود نموذج تدير الفوترة والمطالبات | Draft — Inferred |
| Device and Equipment Integration (Bounded Context) | حدود نموذج تدير استيراد بيانات الأجهزة (يقابل Device Integration Gateway) | Draft — Inferred |
