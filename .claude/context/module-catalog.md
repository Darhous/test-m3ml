...
**الحالة العامة للملف:** الأقسام أدناه Draft (كما كانت) باستثناء قسم جديد
في الأسفل "Confirmed Module Catalog (Post ADR-0012 Acceptance)" — راجعه
أولاً؛ الأقسام القديمة محفوظة للسياق التاريخي فقط ولم تُحذف.
...
**تصنيفات لم تُغطَّ بعد بدليل حقيقي (ملاحظة تاريخية، انظر القسم الجديد في
الأسفل لحالتها بعد ADR-0012):** #4 Patient Modules، #5 Doctor and
...
النضج: **Modeled** (9 contexts) و **Recognized** (19 context). تفاصيل
كاملة في `docs/discovery/artifacts/W9-bounded-context-remapping.md`.

**تحديث (2026-07-18):** ADR-0012 اعتُمِد (Accepted) في مرحلة Open
Questions Resolution — النموذج الهجين (9 Modeled + 19 Recognized = 28)
هو الآن Module Catalog معتمد، وليس Draft. الـ28 السياق يطابقون بنية
`docs/reuse/` وجدول `docs/api-platform/03-API-DOMAIN-INVENTORY.md` تمامًا.
تدرّج الثقة (Modeled أعمق من Recognized) يبقى جزءًا من القرار المعتمد نفسه.
